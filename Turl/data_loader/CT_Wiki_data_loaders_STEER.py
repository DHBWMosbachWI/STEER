...
#     new_valid_headers[table.split("_")[1].split(".csv")[
#         0]] = valid_headers[table]
...
def overwrite_to_one_valid_header(input_data, valid_headers):
    ...
    if valid_header_entry == None:
        #print(f"{table_id} not in valid headers")
        return None
    else:
        #print(f"{table_id} in valid headers")
        column_ids = [int(col.split("_")[1])
                      for col in valid_header_entry.keys()]
        for column_id in column_ids:
            type_annotations[column_id] = [
                valid_header_entry[f"column_{column_id}"]["semanticType"]]

        return [table_id, pgTitle, pgEnt, secTitle, caption, headers, entities, type_annotations]


def filter_labeled_unlabeled_test_data(input_data, config):

    table_id, pgTitle, pgEnt, secTitle, caption, headers, entities, type_annotations = input_data
    if table_id not in config.df["table_id"].unique():
        #print(f"{table_id} not in train data")
        return None
    #print(f"{table_id} in train data")
    labeled_columns = config.df[config.df["table_id"]
                                == table_id]["column"].tolist()
    labeled_columns.sort()
    try:
        # headers = [headers[labeled_column]
        #            for labeled_column in labeled_columns]
        headers = list(map(headers.__getitem__, labeled_columns))
    except:
        print(f"{table_id} in train data\n But headers can not selected.")
        print(f"Headers: {headers}")
        print(f"labeled columns: {labeled_columns}")
    # entities = [entities[labeled_column] for labeled_column in labeled_columns]
    entities = list(map(entities.__getitem__, labeled_columns))
    ...
    # type_annotations = [type_annotations[labeled_column]
    #                     for labeled_column in labeled_columns]
    type_annotations = list(map(type_annotations.__getitem__, labeled_columns))
    return [table_id, pgTitle, pgEnt, secTitle, caption, headers, entities, type_annotations]


def process_single_CT(input_data, config):
    input_data = overwrite_to_one_valid_header(
        input_data, config.valid_headers)
    if input_data == None:
        pass
    else:
        input_data = filter_labeled_unlabeled_test_data(input_data, config)
        if input_data == None:
            pass
        else:
            table_id, pgTitle, pgEnt, secTitle, caption, headers, entities, type_annotations = input_data
            entities = [
                z for column in entities for z in column[:config.max_column]]
            ...
            tokenized_headers = [config.tokenizer.encode(
                z, max_length=config.max_header_length, add_special_tokens=False) for z in headers]
            input_tok = []
            input_tok_pos = []
            input_tok_type = []
            ...
            input_tok_pos += list(itertools.chain(*
                                  [list(range(z)) for z in tokenized_headers_length]))
            ...
            column_entity_mask = np.zeros(
                [len(type_annotations), len(input_ent)], dtype=int)
            ...
            start_i = 0
            header_span = {}
            column_header_mask = np.zeros(
                [len(type_annotations), len(input_tok)], dtype=int)
            for j in range(len(type_annotations)):
                header_span[j] = (start_i, start_i+tokenized_headers_length[j])
                column_header_mask[j, tokenized_meta_length+header_span[j]
                                   [0]:tokenized_meta_length+header_span[j][1]] = 1
                start_i += tokenized_headers_length[j]
            # create input mask
            tok_tok_mask = np.ones([len(input_tok), len(input_tok)], dtype=int)
            meta_ent_mask = np.ones(
                [tokenized_meta_length, len(input_ent)], dtype=int)
            header_ent_mask = np.zeros(
                [sum(tokenized_headers_length), len(input_ent)], dtype=int)

            for e_i, (index, _) in enumerate(entities):
                header_ent_mask[header_span[index[1]][0]
                    :header_span[index[1]][1], e_i] = 1
            ent_header_mask = np.transpose(header_ent_mask)

            input_tok_mask = [tok_tok_mask, np.concatenate(
                [meta_ent_mask, header_ent_mask], axis=0)]
            ent_meta_mask = np.ones(
                [len(input_ent), tokenized_meta_length], dtype=int)
            ...
            for _, e_is in row_en_map.items():
                for e_i_1 in e_is:
                    for e_i_2 in e_is:
                        ent_ent_mask[e_i_1, e_i_2] = 1
            input_ent_mask = [np.concatenate(
                [ent_meta_mask, ent_header_mask], axis=1), ent_ent_mask]
            ...
            input_ent_text = [
                tokenized_pgTitle[:config.max_cell_length]] + input_ent_text
            input_ent_type = [2] + input_ent_type

            new_input_ent_mask = [np.ones([len(input_ent), len(input_tok)], dtype=int), np.ones(
                [len(input_ent), len(input_ent)], dtype=int)]
            ...
            column_entity_mask = np.concatenate(
                [np.zeros([len(type_annotations), 1], dtype=int), column_entity_mask], axis=1)

            input_ent_mask = new_input_ent_mask

            labels = np.zeros(
                [len(type_annotations), config.type_num], dtype=int)
            for j, types in enumerate(type_annotations):
                for t in types:
                    labels[j, config.type_vocab[t]] = 1

            # # one label per col
            # labels = np.zeros([len(type_annotations), 1], dtype=int)
            # for j, typ in enumerate(type_annotations):
            #     labels[j] = config.type_vocab[typ[0]]
            #     #labels.append(config.type_vocab[typ[0]])
            ...
            return [table_id, np.array(input_tok), np.array(input_tok_type), np.array(input_tok_pos), (np.array(input_tok_mask[0]), np.array(input_tok_mask[1])), len(input_tok),
                    np.array(input_ent), input_ent_text_padded, input_ent_cell_length, np.array(
                        input_ent_type), (np.array(input_ent_mask[0]), np.array(input_ent_mask[1])), len(input_ent),
...
class WikiCTDataset(Dataset):
    ...
    def __init__(self, data_dir, entity_vocab, type_vocab, labeled_data_size, unlabeled_data_size, test_data_size, random_state, data_split_set="labeled", add_STEER_train_data=False, max_column=10, max_input_tok=500, src="train", max_length=[50, 10, 10], force_new=False, tokenizer=None):
        ...
        for table in valid_headers.keys():
            new_valid_headers[table.split("_")[1].split(".csv")[
                0]] = valid_headers[table]
        self.valid_headers = new_valid_headers
        ...
        if self.data_split_set == "labeled":
            self.df = pd.DataFrame({"table_id": [entry.split("+")[0].split(".csv")[0].split("_")[1] for entry in self.labeled_unlabeled_test_split_file[f"labeled{self.labeled_data_size}"]],
            ...
            self.df = pd.DataFrame({"table_id": [entry.split("+")[0].split(".csv")[0].split("_")[1] for entry in self.labeled_unlabeled_test_split_file[f"test{self.test_data_size}"]],
