...
def train(args, config, train_dataset, model, eval_dataset=None):
    ...
    optimizer_grouped_parameters = [
        ...
        {'params': [p for n, p in model.table.named_parameters() if (not any(
            nd in n for nd in no_decay))], 'weight_decay': args.weight_decay, 'lr': args.learning_rate},
        {'params': [p for n, p in model.table.named_parameters() if (
            any(nd in n for nd in no_decay))], 'weight_decay': 0.0, 'lr': args.learning_rate},
        {'params': [p for n, p in model.cls.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': args.weight_decay,
         'lr': args.learning_rate*10 if args.cls_learning_rate == 0 else args.cls_learning_rate},
        {'params': [p for n, p in model.cls.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0,
         'lr': args.learning_rate*10 if args.cls_learning_rate == 0 else args.cls_learning_rate}
    ]
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:
                        ...
                        logger.info("  loss = %s", str(
                            (tr_loss - logging_loss)/args.logging_steps))
                        logger.info("  map = %s", str(
                    ...
                    tb_writer.add_scalar('map', (tr_map - logging_map)/(
                        args.gradient_accumulation_steps*args.logging_steps), global_step)
                    logging_map = tr_map
...
def main():
    ...
    # STEER arguments
    parser.add_argument("--labeled_data_size", type=float, default=1)
    parser.add_argument("--unlabeled_data_size", type=str, default="absolute")
    parser.add_argument("--test_data_size", type=float, default=20.0)
    parser.add_argument("--random_state", type=int, default=2)
    parser.add_argument("--add_STEER_train_data", action='store_true', default=False)
    ...
    logging.basicConfig(filename="table_CT_finetuning.log", filemode="w", format='%(asctime)s - %(levelname)s - %(name)s -   %(message)s',
                        datefmt='%m/%d/%Y %H:%M:%S',
                        level=logging.INFO if args.local_rank in [-1, 0] else logging.WARN)
    ...
    if args.do_train:
        ...
        lm_checkpoint = torch.load(os.path.join(
    ...
    if args.do_train:
        ...
        entity_vocab = load_entity_vocab(
            args.data_dir, ignore_bad_title=True, min_ent_count=2)
        ...
        train_dataset = PublicBiCTDataset(args.data_dir, entity_vocab, type_vocab, labeled_data_size=args.labeled_data_size, unlabeled_data_size=args.unlabeled_data_size, test_data_size=args.test_data_size, random_state=args.random_state, data_split_set="labeled", add_STEER_train_data=args.add_STEER_train_data, max_input_tok=500, max_length=[
                                      50, 10, 10], force_new=False, tokenizer=None)
        if args.do_eval or args.evaluate_during_training:
            eval_dataset = PublicBiCTDataset(args.data_dir, entity_vocab, type_vocab, labeled_data_size=args.labeled_data_size, unlabeled_data_size=args.unlabeled_data_size, test_data_size=args.test_data_size, random_state=args.random_state, data_split_set="test", add_STEER_train_data=False, max_input_tok=500, max_length=[
                                        50, 10, 10], force_new=False, tokenizer=None)
        else:
            eval_dataset = None
        assert config.vocab_size == len(train_dataset.tokenizer), \
            "vocab size mismatch, vocab_size=%d" % (
        ...
        global_step, tr_loss = train(
            args, config, train_dataset, model, eval_dataset=eval_dataset)
