...
if __name__ == "__main__":
    ...
    for index, percent in enumerate([1,2,3,4,5]):
        for distance_threshold in [1e-2]:
            comment = f"embclus_lf1_{distance_threshold}_lf2_{cosine_sim_threshold}_labeled{percent}_unlabeledAbsolute_test{test_data_size}_normBatch_no_overwrite"
            ...
            embclus_gen_train_data_path = f"/home/sanonymous/semantic_data_lake/data/extract/out/gen_training_data/{corpus}_gen_training_data_lf1_{distance_threshold}_lf2_{cosine_sim_threshold}_{percent}_absolute_{test_data_size}.csv"
            pretrained_shelock_path = f"sherlock_retrain_embclus_lf1_{distance_threshold}_lf2_{cosine_sim_threshold}_labeled{percent}_unlabeledAbsolute_test{test_data_size}_normBatch_no_overwrite.pt"
            
            if os.environ["TYPENAME"] == "type78":
                pretrained_CRF_LDA_path = f"CRF+LDA_retrain_embclus_lf1_{distance_threshold}_lf2_{cosine_sim_threshold}_labeled{percent}_unlabeledAbsolute_test{test_data_size}.pt"
            else:
                pretrained_CRF_LDA_path = f"CRF+LDA_retrain_embclus_lf1_{distance_threshold}_lf2_{cosine_sim_threshold}_labeled{percent}_unlabeledAbsolute_test{test_data_size}_normBatch_no_overwrite_train100.pt"
            # retrain sherlock
            os.system(
