...
if __name__ == "__main__":
    ...
    for index, percent in enumerate([1,2,3,4,5]):
        for distance_threshold in [1e-6]:
            ...
            # retrain sherlock
            os.system(
                f"python3 ../model/train_sherlock.py -c ../model/params/turl/sherlock_retrain.txt  --comment {comment} --column_level_split_file_path {column_level_split_file_path} --embclus_gen_train_data_path {embclus_gen_train_data_path}"
            )

            # retrain sato
            os.system(
                f"python3 ../model/train_CRF_LC.py -c ../model/params/turl/CRF+LDA_retrain.txt --pre_trained_sherlock_path {pretrained_shelock_path} --comment {comment} --column_level_split_file_path {column_level_split_file_path} --embclus_gen_train_data_path {embclus_gen_train_data_path}"
            )

            # validate sato
            os.system(
                f"python3 ../model/train_CRF_LC.py -c ../model/params/turl/CRF+LDA_eval.txt --model_list {pretrained_CRF_LDA_path} --comment eval_{comment} --column_level_split_file_path {column_level_split_file_path}"
            )
