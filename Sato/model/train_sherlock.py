    ...
    for corpus in corpus_list:
        ...
        if args.mode != 'eval':
            ...
            if (column_level_split_file_path is not None) and (embclus_gen_train_data_path is None):
                ...
                if unlabeled_data_size == "absolute": # if labeled data are in absolute number specified
                    # train = train.set_filter_for_column_level_split(
                    #     labeled_unlabeled_test_split_file[f"labeled{labeled_data_size}"]+labeled_unlabeled_test_split_file[f"unlabeled"])
                    train = train.set_filter_for_column_level_split(
                        labeled_unlabeled_test_split_file[f"labeled{labeled_data_size}"]+df_embclus_training_data["dataset_id"].to_list())
                else:
                    train = train.set_filter_for_column_level_split(
                        labeled_unlabeled_test_split_file[f"labeled{labeled_data_size}"]+labeled_unlabeled_test_split_file[f"unlabeled{unlabeled_data_size}"])
