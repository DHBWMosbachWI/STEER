...
class TableFeatures(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        labels = self.label_enc.inverse_transform(eval(self.df_header.loc[table_id]['field_names']))
    ...
    def set_filter_for_column_level_split(self, column_level_id_filter):
        ...
        # delte duplicated in column_level_id_filter. This should not be the case. 
        # But for testing the "public bi num" this is the case, becasuse we are using the str based gen train data for retraining and there is a overlapping 
        column_level_id_filter = list(dict.fromkeys(column_level_id_filter))
    ...
    def to_col(self):
        ...
        return SherlockDataset(tensor_dict=col_dic, labels=self.label_enc.inverse_transform(labels), label_enc=self.label_enc)
...
class ShuffleFeatures(TableFeatures):
    ...
    def to_col(self):
        ...
        return ShuffleFeaturesCol(tensor_dict=col_dic, labels=self.label_enc.inverse_transform(labels), label_enc=self.label_enc)
