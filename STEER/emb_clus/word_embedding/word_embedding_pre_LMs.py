...
tokenizer = BertTokenizer.from_pretrained(pre_trained_LM)
model = BertModel.from_pretrained(pre_trained_LM)
...
def embedding_for_one_col(column: list):
    ...
    #embedding with pre-trained LM
    col_tokenized = tokenizer.encode(", ".join(column), return_tensors="pt", max_length=500)
    embedding = model(col_tokenized)
    embedding = embedding[0][0][0].detach().tolist()
...
def extract_embedding_context_aware_representation(n_samples:int, df_dic: dict):
    df, domain, table = df_dic["df"], df_dic["domain"], df_dic["table"]

    if (n_samples != None) & (len(df) > n_samples):
        df = df.sample(n_samples, replace=True)

    max_length = int(512/len(df.columns))
    ...
    CLS_index_list = [0]
    for col_num, col_name in enumerate(df.columns):
        cur_col = tokenizer.encode(", ".join(df[col_name].apply(str).tolist()), max_length=max_length)
        if col_num < (len(df.columns)-1):
            tokenized_table.extend(cur_col[:-1])
            CLS_index_list.append(len(cur_col[:-1]))
        else:
            tokenized_table.extend(cur_col)
    CLS_index_list = np.cumsum(CLS_index_list).tolist()

    table_representation = model(torch.LongTensor([tokenized_table]))
    ...
    for col_num, col_name in enumerate(df.columns):
        ...
        # get embedding
        all_results[col_num]["word_embedding"] = table_representation[0][0][CLS_index_list[col_num]].tolist()
        all_results[col_num]["used_col_values"] = len(df[col_name])
...
if __name__ == "__main__":
    ...
    parser.add("--approach", type=str, default="single_col")
    ...
    output_file_path = join(os.environ["WORKING_DIR"], "emb_clus",
                            "word_embedding", "out",
                            f"{corpus}_{fraction}_{pre_trained_LM}_{approach}.csv")
    ...
    for raw_df in raw_df_iter:
        if approach == "single_col":
            df_features = extract_word_embedding(n_samples, raw_df)
        elif approach == "context_aware":
            df_features = extract_embedding_context_aware_representation(n_samples, raw_df)
        counter += 1
